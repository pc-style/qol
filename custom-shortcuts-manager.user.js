...
(function() {
  ...
  function loadAll() {
    const stored = safeParse(GM_getValue(STORAGE_KEY, null), null);
    const storedSettings = safeParse(GM_getValue(SETTINGS_KEY, null), null);
  ...
  function persist() {
    ...
    GM_setValue(STORAGE_KEY, JSON.stringify(rest));
  ...
  function showToast(message, type = 'info', label) {
    ...
    el.className = 'csm-toast';
    if (type === 'error') el.classList.add('csm-error');
    if (type === 'success') el.classList.add('csm-success');
    const labelText = label || (type === 'error' ? 'Conflict' : type === 'success' ? 'Done' : 'Shortcuts');
    el.innerHTML = `<span class="csm-toast-label">${labelText}</span><span>${message}</span>`;
